      ...
      {status === "Completed" && (
        <div className="space-y-3">
          <p className="text-sm text-[var(--accent)]">
            Your compressed model is ready.
          </p>
          <button
            onClick={() =>
              navigate(`/results/${jobId}`, { state: location.state })
            }
            className="w-full px-4 py-2.5 bg-[var(--accent)] text-black text-sm font-medium rounded-lg hover:brightness-110 transition-all"
          >
            View results
          </button>
        </div>
      ...
      {(status === "Failed" || status === "Stopped") && (
        <div className="bg-red-950 border border-red-800 rounded-lg p-4">
          ...
          <p className="mt-2 text-xs text-red-500">
            ...
            <span className="font-mono">{jobId}</span> for details.
          </p>
        </div>
      )}

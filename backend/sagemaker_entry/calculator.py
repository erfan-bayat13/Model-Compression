...
def _calculate_width_targets(
    ...
    """
    logger.debug(f"DEBUG: num_heads={num_heads}, num_kv_groups={num_kv_groups}")
    logger.debug(
        f"DEBUG: head_divisor={math.lcm(num_heads, num_kv_groups)}, combined={math.lcm(math.lcm(num_heads, num_kv_groups), alignment)}"
    )
    logger.debug(
        f"DEBUG: raw_hidden={hidden_size * math.sqrt(1.0 - width_pruning_pct):.2f}"
    )
...
def calculate_compression_targets(
    ...
    original_counts = _count_params(
        num_layers, hidden_size, ffn_size, num_heads, num_kv_groups, vocab_size
    )
